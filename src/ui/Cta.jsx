...
function Cta({ color, text }) {
  return (
    <motion.div
      className="group flex cursor-pointer items-center gap-12 text-[1.2rem] font-bold uppercase tracking-[0.2rem]"
      initial={{ columnGap: "3rem" }}
      whileHover={{ columnGap: "1rem" }}
      transition={{ type: "spring", duration: 0.3, bounce: 0.5 }}
    >
      <Link className="transition-all duration-300 group-hover:underline">
        {text}
      </Link>
